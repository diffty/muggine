...
GameManager* GameManager::s_pInstance = NULL;


GameManager::GameManager() {
	GameManager::s_pInstance = this;
	...
	// Instantiate menu
	m_pMainMenu = new XXMainMenu(&m_menuScene);

	m_pSys = System::get();
	m_pInputSys = System::get()->getInputSys();

	/*m_pMainMenu->setNewBtnCallback(GameManager::newGameMenuBtnCallback, this);
	m_pMainMenu->setQuitBtnCallback(GameManager::quitGameMenuBtnCallback, this);
	m_pMainMenu->setContinueBtnCallback(GameManager::continueGameMenuBtnCallback, this);*/
...
void GameManager::newGameMenuBtnCallback(void* pObj) {
	((GameManager*)pObj)->newGame();
}

void GameManager::quitGameMenuBtnCallback(void* pObj) {
	((GameManager*)pObj)->quitGame();
}

void GameManager::continueGameMenuBtnCallback(void* pObj) {
	((GameManager*)pObj)->hideMenu();
}
...
void GameManager::onNewLevel(bool bReplayLevel) {
	changeState(E_APP_STATE_LEVEL_BEGIN, true);

	if (m_pLevelBeginScreen) delete m_pLevelBeginScreen;

	m_pLevelBeginScreen = new XXLevelBeginScreen(0);
...
void GameManager::onLevelSuccess() {
	changeState(E_APP_STATE_LEVEL_SUCCESS, true);
	if (m_pLevelSuccessScreen) delete m_pLevelSuccessScreen;
	m_pLevelSuccessScreen = new XXLevelSuccessScreen(0);
}

void GameManager::onLevelFail() {
	changeState(E_APP_STATE_LEVEL_FAIL, true);
	if (m_pLevelFailScreen) delete m_pLevelFailScreen;
	m_pLevelFailScreen = new XXLevelFailScreen();
}

void GameManager::onStartLevel() {
	GameMode::initGameMode(&m_pGameMode, &m_gameScene);
	changeState(E_APP_STATE_INGAME, false);
}

void GameManager::onEndLevel() {
    m_pGameOverScreen = new XXGameOverScreen();
    changeState(E_APP_STATE_LEVEL_END, false);
}

void GameManager::onEndTransition(ETransitionAnimType eTransType) {
	switch (eTransType) {
	case TRANSITION_IN:
...
void GameManager::changeState(E_APP_STATE eNewState, bool bWithTransition) {
	if (bWithTransition) {
...
void GameManager::fadeScreen(E_FADE_MODE eFadeMode, float fDuration) {
...
Scene* GameManager::getMenuScene() {
	return &m_menuScene;
}


E_APP_STATE GameManager::getCurrentState() {
	return m_eCurrState;
}
...
GameManager* GameManager::get() {
	return GameManager::s_pInstance;
}
